...
class FireStoreChatController {
  ...
  Future<ProcessResponse<Chat>> checkChat({required String peerUid}) async {
    //
    QuerySnapshot<Chat> querySnapshot = await getChatByPeer(peerUid: peerUid);

    if (querySnapshot.docs.isNotEmpty) {
      //CHAT EXISTS!!!!
      print('CHat Exist: ID: ${querySnapshot.docs[0]
          .data()
          .id}');
      return ProcessResponse(
          message: 'Success',
          success: true,
          object: querySnapshot.docs[0].data());
    } else {
      //CREATE NEW CHAT
  ...
  Future<QuerySnapshot<Chat>> getChatByPeer({required String peerUid}) async {
...
// Fire store interception with each other that caused ignore inside with-converted with outside with-converted
  Stream< List<Chat> /*QuerySnapshot<Chat>*/> getChats() async* {
    ...
    print("yes");
    print("Start Stream");
    Stream<QuerySnapshot<Chat>> chatStream = collectionReference
        .withConverter<Chat>(
        fromFirestore: (snapshot, options) {
          print("=============getchat================");
  ...
  Future<List<Chat>> getFuture(List<QueryDocumentSnapshot<Chat>> data) async {
    ...
    print("data:$data");
    print('123');
    List<Chat> chats = data.map((e) {print(e.data().peer1Uid);return e.data();}).toList();
    print('456');
    print(chats);
    ...
    for (Chat chat in chats) {
      ...
      ChatUser? chatUser = await FirestoreUserController().getUserById(id:getMyRefFriends(chat));
      print("object");
